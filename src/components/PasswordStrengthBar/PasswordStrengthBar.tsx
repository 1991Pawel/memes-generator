...
const passwordType = {
  week: { message: "Słabe", color: "#E5E4E2", colorBar: 1 },
...
export const PasswordStrengthBar = ({ passwordValue }: any) => {
  ...
  const getActiveColor = (type: string) => {
    if (type === passwordType.week.message) return passwordType.week.color;
    if (type === passwordType.medium.message) return passwordType.medium.color;
    return passwordType.strong.color;
  };
      ...
      {barNumber.map((_, i) => {
        return (
          <span
            ...
            style={{
              background:
                activeStrength.colorBar > i
                  ? activeStrength.color
                  : passwordType.week.color,
            }}
            key={i}
          ></span>
        );
